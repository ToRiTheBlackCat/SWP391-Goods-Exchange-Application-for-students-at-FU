...
import Filter from './Filter'; // Assuming the Filter component is in the same directory
...
function Category({ onCategorySelect, selectedCategoryId, onSortChange }) {
  return (
    <div className={styles.App}>
      <Filter onSortChange={onSortChange} /> {/* Render the Filter component */}
          ...
          <Row>
            {categories.map((category) => (
              <Col md={4} key={category.id} className={`mb-4 d-flex ${styles.col}`}>
                <Card 
                  className={`${styles.card} ${selectedCategoryId === category.id ? styles.selected : ''}`} 
